...
#优化参数
pricechange = 0.0025，interval_s=10
...
import numpy as np
########################################################################
class Track02Strategy(CtaTemplate):
    """基于Tick的交易策略"""
    className = 'Track02Strategy'
    author = '南石资本'

    count = 0#记时

    # 策略参数
    pricechange_order = 0.0025
    pricechange_stop = 0
    volumechange =0
    fixed_size = 1
    interval_s=10
    ...
    # 参数列表，保存了参数的名称
    parameters = ["pricechange_order","pricechange_stop","volumechange","interval_s","fixed_size","slip","decimal"]
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        super(Track02Strategy, self).__init__(cta_engine, strategy_name, vt_symbol, setting)
    ...
    def on_tick(self, tick: TickData):
        ...
        if self.pos == 0:
            if ta.RatePrice()>self.pricechange_order and ta.RateVolume()>self.volumechange:
            ...
            if ta.RatePrice()<-1*self.pricechange_order and ta.RateVolume()>self.volumechange:
        ...
        if self.pos > 0:
            if ta.RatePrice()<-1*self.pricechange_stop:
                self.sell(float(self.decimal%(ta.TicklastPriceArray[-1]*(1-self.slip))), self.fixed_size, False)
                self.write_log("sell order"+str(ta.TicklastPriceArray[-1]))
        if self.pos < 0:
            if ta.RatePrice()>self.pricechange_stop:
                self.cover(float(self.decimal%(ta.TicklastPriceArray[-1]*(1+self.slip))), self.fixed_size, False)
