    ...
    def on_tick(self, tick: TickData):
        ...
        if self.pos == 0:
            if ta1.RatePrice()>self.pricechange_order and ta1.RateVolume()>self.volumechange:
                self.buy(float(self.decimal%(ta1.TicklastPriceArray[-1]*(1+self.slip))), self.fixed_size, False)
                self.write_log("buy order"+str(ta1.TicklastPriceArray[-1]))
            if ta1.RatePrice()<-1*self.pricechange_order and ta1.RateVolume()>self.volumechange:
                self.short(float(self.decimal%(ta1.TicklastPriceArray[-1]*(1-self.slip))), self.fixed_size, False)
                self.write_log("short order"+str(ta1.TicklastPriceArray[-1]))
        if self.pos > 0:
            if ta2.RatePrice()<-1*self.pricechange_stop or ta.TicklastPriceArray[-1]<self.posPrice*(1-self.losestop):
                self.sell(float(self.decimal%(ta2.TicklastPriceArray[-1]*(1-self.slip))), self.pos, False)
                self.write_log("sell order"+str(ta2.TicklastPriceArray[-1]))
        if self.pos < 0:
            if ta2.RatePrice()>self.pricechange_stop or ta.TicklastPriceArray[-1]>self.posPrice*(1+self.losestop):
                self.cover(float(self.decimal%(ta2.TicklastPriceArray[-1]*(1+self.slip))), -self.pos, False)
                self.write_log("cover order"+str(ta2.TicklastPriceArray[-1]))
